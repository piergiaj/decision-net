...
if __name__ == '__main__':
    ...
    for ind in [1,2,3,4]: # for each of the 4 cases, load the seat network
        ...
        u = evaluate_network(json.loads(case1[0]), json.loads(case1[1]), json.loads(case1[2]))
        ...
        from plot import exp4
        exp4(np.asarray(knowledge), np.asarray(no_knowledge), np.asarray(complexity, dtype='float64'), np.asarray(utility), ind)
    ...
    from plot import exp4_full_scatter
    exp4_full_scatter(np.asarray(total_knowledge), np.asarray(total_complexity, dtype='float64'), np.asarray(total_utility))
